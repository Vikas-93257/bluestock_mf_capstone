...
# --- find project root automatically (works Windows + Mac) ---
...
# ---- check all 10 files exist first ----
files = {
...
if missing:
    print("\nERROR: Missing files! Copy all 10 CSVs into data/raw first.")
...
for name, fname in files.items():
    ...
    try:
        ...
        if dups > 0:
            anomalies.append(f"{name}: {dups} duplicates")
...
if "fund_master" in dfs:
    ...
    print("\n  Sample 5 codes:")
    print(fm[["amfi_code", "scheme_name", "fund_house"]].head(5).to_string(index=False))

    print("\n8. SEBI Category Codes:")
    print(fm["sebi_category_code"].value_counts())
else:
    print("fund_master not loaded – cannot run Task 6")
...
if "fund_master" in dfs and "nav_history" in dfs:
    ...
    if len(missing_in_nav) == 0 and len(extra_in_nav) == 0:
        ...
        print("\n>>> AMFI VALIDATION FAILED <<<")
        anomalies.append("AMFI code mismatch found")
    ...
    for tname in ["scheme_performance", "investor_transactions", "portfolio_holdings"]:
        if tname in dfs and "amfi_code" in dfs[tname].columns:
            ...
            if orphan:
                print(f"  Orphan: {sorted(list(orphan))[:5]}")
...
# Known expected anomaly from Bluestock data
